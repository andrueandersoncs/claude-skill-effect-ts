...
import type { CategoryDetector, DetectorConfig } from "./types.js";
import { DetectorResult, type Violation } from "./types.js";
 ...
 */
const matchesAny = (patterns: readonly string[], filePath: string): boolean => {
...
export const detectDirectory = (
	...
	// Use mutable arrays during processing
	let filesAnalyzed = 0;
	const violations: Violation[] = [];
	...
	for (const file of files) {
		filesAnalyzed++;
		const { violations: fileViolations, error } = detectFile(file, detectors);

		if (error) {
			errors.push({ filePath: file, error });
		}
	...
	// Create immutable result at the end
	return new DetectorResult({ filesAnalyzed, violations, errors });
};
